...
void* Reader(void *arg)
{
    sleep(1);
    int temp=(int)arg;
    printf("\nReader %d is trying to enter into the Databse for reading the data ",temp);
...
int main()
{
    int i=0,ReaderThread=0,WriterThread;
    sem_init(&readCountAccess,0,1);
    sem_init(&databaseAccess,0,1);

    pthread_t Readers_thr[100],Writer_thr[100];
    printf("\nEnter number of Readers thread(MAX 10) ");
    scanf("%d",&ReaderThread);
    printf("\nEnter number of writers tread(MAX 10) ");
    scanf("%d",&WriterThread);

    for(i=0;i<ReaderThread;i++)
        pthread_create(&Readers_thr[i],NULL,Reader,(void*)i);

    for(i=0;i<WriterThread;i++)
